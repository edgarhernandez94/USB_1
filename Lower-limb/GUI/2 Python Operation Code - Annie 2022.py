...
def create_window():
    window = tk.Tk()
    window.title("   Annie Exoskeleton")
...
def init_images(app, init_position, init_sticky, init_rowspan, init_columnspan, init_padding, init_ipadding):
    global annie_not_connected, Annie_not_connected
    global annie_stand_waiting, Annie_stand_waiting
    global annie_sit_waiting, Annie_sit_waiting
    global annie_stand_straight, Annie_stand_straight
    global plus, Plus
    global minus, Minus
    global annie_sit_straight, Annie_sit_straight
    global annie_gait_training, Annie_gait_training
    global annie_step_one, annie_step_two, annie_step_three, annie_step_four, annie_step_five
    global annie_standing_exercises, Annie_standing_exercises
    global annie_left_backwards, annie_left_forwards
    global annie_right_backwards, annie_right_forwards
    global annie_sitting_exercises, Annie_sitting_exercises
    global annie_left_bend, annie_left_extend
    global annie_right_bend, annie_right_extend
    global annie_both_bend, annie_both_extend

    annie_not_connected = Image.open("Annie_not_connected.png")
    annie_not_connected = annie_not_connected.resize((180,230), Image.ANTIALIAS)
    annie_not_connected = ImageTk.PhotoImage(master=app, image=annie_not_connected)
    Annie_not_connected = image_label(init_position, init_sticky, init_rowspan, init_columnspan, init_padding, init_ipadding,
                                      master=app, image=annie_not_connected, bd=0)

    annie_stand_waiting = Image.open("Annie_stand_waiting.png")
    annie_stand_waiting = annie_stand_waiting.resize((110,200), Image.ANTIALIAS)
    annie_stand_waiting = ImageTk.PhotoImage(master=app, image=annie_stand_waiting)
    Annie_stand_waiting = image_label(init_position, init_sticky, init_rowspan, init_columnspan, init_padding, init_ipadding,
                                      master=app, image=annie_stand_waiting, bd=0)
    ...
    annie_sitting_exercises = Image.open("Annie_sitting_exercises.png")
    annie_sitting_exercises = annie_sitting_exercises.resize((190,190), Image.ANTIALIAS)
    annie_sitting_exercises = ImageTk.PhotoImage(master=app, image=annie_sitting_exercises)
    Annie_sitting_exercises = image_label(init_position, init_sticky, init_rowspan, init_columnspan, init_padding, init_ipadding,
                                       master=app, image=annie_sitting_exercises, bd=0)

    annie_right_bend = Image.open("Annie_right_bend.png")
    annie_right_bend = annie_right_bend.resize((190,190), Image.ANTIALIAS)
    annie_right_bend = ImageTk.PhotoImage(master=app, image=annie_right_bend)
    annie_right_extend = Image.open("Annie_right_extend.png")
    annie_right_extend = annie_right_extend.resize((190,190), Image.ANTIALIAS)
    annie_right_extend = ImageTk.PhotoImage(master=app, image=annie_right_extend)
...
def init_shared_buttons(app, init_position, init_sticky, init_rowspan, init_columnspan, init_padding, init_ipadding):
    ...
    mainMenuTitle = label(init_position, init_sticky, init_rowspan, init_columnspan, init_padding, init_ipadding,
                          size=20, master=app, text='Annie Exoskeleton', bg='white', fg='black')
...
def init_gait_menu_buttons(app, init_position, init_sticky, init_rowspan, init_columnspan, init_padding, init_ipadding):
    ...
    buttonStepOne_GaitMenu = button((1,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (10,0),
                           ...
                           command = lambda j="1":[Annie_gait_training.config(image=annie_step_one), send_instruction(j,labelStateProcess,1,1),enable_buttons([buttonStepTwo_GaitMenu]),disable_buttons([buttonStepFour_GaitMenu,buttonStepFive_GaitMenu])])
    buttonStepTwo_GaitMenu = button((2,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (10,0),
                           ...
                           command = lambda j="2":[Annie_gait_training.config(image=annie_step_two),send_instruction(j, labelStateProcess,1,1),enable_buttons([buttonStepThree_GaitMenu]),disable_buttons([buttonStepOne_GaitMenu])])
    buttonStepThree_GaitMenu = button((3,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (10,0),
                             ...
                             command = lambda j="3":[Annie_gait_training.config(image=annie_step_three),send_instruction(j,labelStateProcess,1,1),enable_buttons([buttonStepFour_GaitMenu,buttonStepFive_GaitMenu]),disable_buttons([buttonStepTwo_GaitMenu])])
    buttonStepFour_GaitMenu = button((4,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (10,0),
                            ...
                            command = lambda j="4":[Annie_gait_training.config(image=annie_step_four),send_instruction(j,labelStateProcess,1,1),enable_buttons([buttonStepOne_GaitMenu]),disable_buttons([buttonStepThree_GaitMenu,buttonStepFive_GaitMenu])])
    buttonStepFive_GaitMenu = button((5,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (10,0),
                            ...
                            command = lambda j="5":[Annie_gait_training.config(image=annie_step_five),send_instruction(j,labelStateProcess,1,1),enable_buttons([buttonStepOne_GaitMenu]),disable_buttons([buttonStepThree_GaitMenu,buttonStepFour_GaitMenu])])
    ...
    buttonConfirmStand_GaitMenu = button((0,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (15,0),
                                 ...
                                 command = lambda j="S" :[Annie_gait_training.config(image=annie_gait_training),send_instruction(j,labelStateProcess,1,1),enable_buttons([buttonStepOne_GaitMenu]),disable_buttons([buttonStepTwo_GaitMenu,buttonStepThree_GaitMenu,buttonStepFour_GaitMenu,buttonStepFive_GaitMenu])])
...
def init_standing_exercises_menu_buttons(app, init_position, init_sticky, init_rowspan, init_columnspan, init_padding, init_ipadding):
    ...
    buttonLeftBackwards = button((1,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (15,0),
                           ...
                           command = lambda j="A": [Annie_standing_exercises.config(image=annie_left_backwards), send_instruction(j,labelStateProcess,1,1), enable_buttons([buttonConfirmStand], button_color=indi_purple), disable_buttons([buttonLeftBackwards,buttonLeftForwards,buttonRightBackwards,buttonRightForwards])])
    buttonLeftForwards = button((2,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (20,0),
                                ...
                                command=lambda j="B": [Annie_standing_exercises.config(image=annie_left_forwards), send_instruction(j,labelStateProcess,1,1), enable_buttons([buttonConfirmStand], button_color=indi_purple), disable_buttons([buttonLeftBackwards,buttonLeftForwards,buttonRightBackwards,buttonRightForwards])])
    buttonRightBackwards = button((1,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (15,0),
                           ...
                           command = lambda j="C": [Annie_standing_exercises.config(image=annie_right_backwards), send_instruction(j,labelStateProcess,1,1), enable_buttons([buttonConfirmStand], button_color=indi_purple), disable_buttons([buttonLeftBackwards,buttonLeftForwards,buttonRightBackwards,buttonRightForwards])])
    buttonRightForwards = button((2,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (20,0),
                                ...
                                command=lambda j="D": [Annie_standing_exercises.config(image=annie_right_forwards), send_instruction(j,labelStateProcess,1,1), enable_buttons([buttonConfirmStand], button_color=indi_purple), disable_buttons([buttonLeftBackwards,buttonLeftForwards,buttonRightBackwards,buttonRightForwards])])
...
def init_sitting_exercises_menu_buttons(app, init_position, init_sticky, init_rowspan, init_columnspan, init_padding, init_ipadding):
    ...
    buttonLeftBend = button((2,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (20,0),
                           ...
                           command = lambda j="E": [Annie_sitting_exercises.config(image=annie_left_bend), send_instruction(j,labelStateProcess,1,1)])
    buttonLeftExtend = button((1,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (15,0),
                                ...
                                command=lambda j="F": [Annie_sitting_exercises.config(image=annie_left_extend), send_instruction(j,labelStateProcess,1,1)])
...
##
    buttonRightBend = button((2,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (20,0),
                           ...
                           command = lambda j="0": [Annie_sitting_exercises.config(image=annie_right_bend), send_instruction(j,labelStateProcess,1,1)])
##
##
    buttonRightExtend = button((1,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (15,0),
                                ...
                                command=lambda j="9": [Annie_sitting_exercises.config(image=annie_right_extend), send_instruction(j,labelStateProcess,1,1)])
##
##
    ...
    buttonBothBend = button((1,1), init_sticky, init_rowspan, init_columnspan, init_button_padding, (20,0),
                           ...
                           command = lambda j="I": [Annie_sitting_exercises.config(image=annie_both_bend), send_instruction(j,labelStateProcess,1,1)])
    buttonBothExtend = button((1,0), init_sticky, init_rowspan, init_columnspan, init_button_padding, (15,0),
                           ...
                           command = lambda j="J": [Annie_sitting_exercises.config(image=annie_both_extend), send_instruction(j,labelStateProcess,1,1)])
...
def waiting_for_connection():
    ...
    Annie_not_connected.position = (2,0)
    frameConnection.position = (3,0)
    frameBottom.position = (4,0)
    create_grid(app, [mainMenuTitle,labelStateProcess,Annie_not_connected,frameConnection,frameBottom],
                rows=5, columns=1)
...
def sitting_exercises_menu():
    ...
    Annie_sitting_exercises.config(image=annie_sitting_exercises)
    ...
    buttonStartRoutine.config(command = lambda j="r" : [Annie_sitting_exercises.config(image=annie_sitting_exercises), send_instruction(j,labelStateProcess,1,1)])
    ...
    frameLeftLeg.position, Annie_sitting_exercises.position, frameRightLeg.position = (3,0), (3,1), (3,2)
    frameBothLegs.position, frameBothLegs.columnspan = (4,0), 3
    frameBottom.position, frameBottom.columnspan = (5,0), 3
    create_grid(app, [mainMenuTitle,labelStateProcess,frameLeftLeg,Annie_sitting_exercises,buttonStartRoutine,frameRightLeg,frameBottom],
                rows=6, columns=3)
